...
export default class BasketRouter extends ApplicationRouter {
    ...
    constructor(basketRequester: IBasketRequester, checkStoreAccessMiddleware: e.RequestHandler, extractOpenBasketIdMiddleware: e.RequestHandler) {
        super();
    ...
    public initRoutes() {
        this.getRouter().post('/basket/:productOptionId', this.checkStoreAccessMiddleware, this.extractOpenBasketIdMiddleware, async(req: any, res: any) => {
        ...
        this.getRouter().get('/basket', this.checkStoreAccessMiddleware, this.extractOpenBasketIdMiddleware, async(req: any, res: any) => {
            const basketId = req.basketId;
            const productOptions = await this.basketRequester.getBasketProductOptions(basketId);
            res.send(productOptions);
        });

        this.getRouter().put('/basket/:productOptionId', this.checkStoreAccessMiddleware, this.extractOpenBasketIdMiddleware, async(req: any, res: any) => {
            const basketId = req.basketId;
            const productOptionId = String(req.params.productOptionId);
            const quantity = req.body.quantity;
            const basketProductOption = new BasketProductOptionDS(basketId, productOptionId, quantity);
        ...
        this.getRouter().delete('/basket/:productOptionId', this.checkStoreAccessMiddleware, this.extractOpenBasketIdMiddleware, async(req: any, res: any) => {
            const basketId = req.basketId;
            const productOptionId = String(req.params.productOptionId);
